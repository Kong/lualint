...
define_update_trailing_trivia!(FunctionBody, |this, trailing| {
    this.to_owned().with_end_token(this.end_token().update_trailing_trivia(trailing))
});
...
define_update_trivia!(Attribute, |this, leading, trailing| {
    this.to_owned().with_brackets(this.brackets().update_trivia(leading, trailing))
});
    ...
    match this {
        ...
        Stmt::FunctionDeclaration(stmt) => {
            ...
            Stmt::FunctionDeclaration(
                stmt.to_owned().with_function_token(function_token).with_body(body),
            )
        }
        ...
        Stmt::ExportedTypeDeclaration(stmt) => {
            ...
            let type_declaration = stmt.type_declaration().to_owned().with_type_definition(
                stmt.type_declaration().type_definition().update_trailing_trivia(trailing),
            );
...
define_update_leading_trivia!(Value, |this, leading| {
    match this {
        Value::Function((token, function_body)) => {
            Value::Function((token.update_leading_trivia(leading), function_body.to_owned()))
        }
...
define_update_trailing_trivia!(Value, |this, trailing| {
    match this {
        Value::Function((token, function_body)) => {
            Value::Function((token.to_owned(), function_body.update_trailing_trivia(trailing)))
        }
        Value::FunctionCall(function_call) => {
...
define_update_trivia!(TypeInfo, |this, leading, trailing| {
    match this {
        ...
        TypeInfo::Module { module, punctuation, type_info } => TypeInfo::Module {
        ...
        TypeInfo::Typeof { typeof_token, parentheses, inner } => TypeInfo::Typeof {
...
define_update_leading_trivia!(TypeArgument, |this, leading| {
    if let Some((name, colon)) = this.name() {
        this.to_owned().with_name(Some((name.update_leading_trivia(leading), colon.to_owned())))
    } else {
        this.to_owned().with_type_info(this.type_info().update_leading_trivia(leading))
    }
});
...
define_update_trailing_trivia!(TypeSpecifier, |this, trailing| {
    this.to_owned().with_type_info(this.type_info().update_trailing_trivia(trailing))
});
...
define_update_trailing_trivia!(GenericDeclarationParameter, |this, trailing| {
    if let Some(default_type) = this.default_type() {
        ...
        this.to_owned().with_default(Some((this.equals().unwrap().to_owned(), default_type)))
    } else {
        let parameter_info = match this.parameter() {
            GenericParameterInfo::Name(token) => {
...
define_update_trailing_trivia!(IfExpression, |this, trailing| {
    this.to_owned().with_else(this.else_expression().update_trailing_trivia(trailing))
});

#[cfg(feature = "luau")]
define_update_leading_trivia!(ElseIfExpression, |this, leading| {
    this.to_owned().with_else_if_token(this.else_if_token().update_leading_trivia(leading))
});
...
define_update_leading_trivia!(InterpolatedString, |this, leading| {
    ...
    if let Some(segment) = segment_iter.next() {
        ...
        this.to_owned().with_last_string(this.last_string().update_leading_trivia(leading))
    }
});

#[cfg(feature = "luau")]
define_update_trailing_trivia!(InterpolatedString, |this, trailing| {
    this.to_owned().with_last_string(this.last_string().update_trailing_trivia(trailing))
});
